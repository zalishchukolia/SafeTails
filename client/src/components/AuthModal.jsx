  ...
  return (
    <>
      ...
      <div style={{
        ...
        zIndex: 501, width: '100%', maxWidth: 400,
        background: '#1a1a1a', borderRadius: 20,
        border: '1px solid #2a2a2a', padding: '36px 32px',
        fontFamily: font,
      }}>
        {/* Close */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {mode === 'register' && (
            <input name="name" value={form.name} onChange={handleChange}
              placeholder="Ваше імʼя" style={inputStyle} />
          )}
          <input name="email" value={form.email} onChange={handleChange}
            placeholder="Email" type="email" style={inputStyle} />
          <input name="password" value={form.password} onChange={handleChange}
            placeholder="Пароль" type="password" style={inputStyle} />
      ...
      <style>{`input:focus { outline: none; border-color: #ff6b2b !important; }`}</style>
    </>
  )
}
