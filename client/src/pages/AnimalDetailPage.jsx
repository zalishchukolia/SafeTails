...
function AnimalDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://safetails-production-8790.up.railway.app/api/animals/${id}`
    )
      .then((res) => res.json())
      .then((data) => {
        setAnimal(data)
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.12fr)_minmax(0,0.88fr)] gap-6">
              ...
              <article className="rounded-[28px] bg-[#f4efe3] text-[#2a261e] p-6 border border-black/5">
                ...
                <p className="text-sm sm:text-base leading-7 text-[#4f4638]">
                  {animal.description ||
                    'This animal came into care with a detailed history, received treatment, and is now being prepared for adoption.'}
                </p>
